...
class NotToDoListCell: UICollectionViewCell {
    ...
    let textField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Not To Do Task"
    ...
    override init(frame: CGRect) {
        ...
        tableViewList.register(UITableViewCell.self, forCellReuseIdentifier: ntdlCell)
        setUpViews()
...
extension NotToDoListCell: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if (editingStyle == .delete) {
            ntdlArray.remove(at: indexPath.row)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = ntdlArray[indexPath.row]
